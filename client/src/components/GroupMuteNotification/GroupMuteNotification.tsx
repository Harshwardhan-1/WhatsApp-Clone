...
import "../muteNotification/mute.notification.css";
...
export function GroupMuteNotification({onBack,_id,senderId}:prop){
    const [muted,setMuted]=useState("off");


    useEffect(()=>{
        socket.emit("prev_mark_group_notification",{_id,senderId});
        socket.on("set_prev_mark_group_notification",(duration:string)=>{
            setMuted(duration || "off");
        });
        socket.on("group_duration_updated",(duration:string)=>{
            setMuted(duration);
        });
        
        return()=>{
            socket.off("set_prev_mark_group_notification");
            socket.off("group_duration_updated");
        }
    },[]);
...
const handleChange=async(e:React.ChangeEvent<HTMLInputElement>)=>{
    try{
        ...
        socket.emit("change_group_notification",({_id,senderId,duration:value}));
        console.log(value);
    }catch(err:any){
        console.log(err);
        showMessage(err);
