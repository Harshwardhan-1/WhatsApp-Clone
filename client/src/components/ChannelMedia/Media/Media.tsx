...
import "../../ChatMedia/media/media.css";
...
export function ChannelMedia({onBack,channelId,senderId}:prop){
    ...
    useEffect(()=>{
        socket.emit("all_channel_media",({channelId,senderId}));
        socket.on("got_all_channel_media",handleAllMedia);

        return()=>{
            socket.off("got_all_channel_media",handleAllMedia);
        }
    },[]);
    return(
