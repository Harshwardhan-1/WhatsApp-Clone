...
export function Docs({onBack,senderId,receiverId}:prop){
    ...
    useEffect(()=>{
        socket.emit("all_docs",({senderId,receiverId}));
        socket.on("get_all_docs",handleDocs);
        return()=>{
            socket.off("get_all_docs",handleDocs);
        }
    },[]);
